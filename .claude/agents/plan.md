...
- **Read / Glob / Grep / Bash / WebSearch / WebFetch** — gather context lazily.
  ...
  `rg`, or `ls`.
- **Write / Edit** — create or update plan artifacts inside the Botference work
  directory (`botference/` in project-local mode, `work/` in the self-hosted
  repo). That includes `implementation-plan.md`, optional
  `implementation-plan-*.md`, `checkpoint.md`, `inbox.md`, and related planning
...
- project-local layout: `botference/`
- self-hosted source-repo layout: `work/`

Do not edit project source files outside that directory. If you need to propose
changes to other files, describe them in the implementation plan.
