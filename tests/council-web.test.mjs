...
test('sidebar chat row: ⋯ opens archive + delete, both send the plain slash command',
  ...
  await new Promise(r => setTimeout(r, 5));
  assert.deepEqual(posts.pop(), { url: '/input', body: { text: '/archive sidB', attachments: [] } });
  ...
  await new Promise(r => setTimeout(r, 5));
  assert.deepEqual(posts.pop(), { url: '/input', body: { text: '/delete sidB', attachments: [] } });

  // tapping the row itself still just resumes it
...
test('sidebar projects: archived ones collapse into their own section; archive/unarchive send /project commands',
  ...
  await new Promise(r => setTimeout(r, 5));
  assert.deepEqual(posts.pop(), { url: '/input', body: { text: '/project unarchive old', attachments: [] } });

  // and the live project can be archived from its own body
  doc.querySelector('[data-act="proj-archive"]').click();
  await new Promise(r => setTimeout(r, 5));
  assert.deepEqual(posts.pop(), { url: '/input', body: { text: '/project archive live', attachments: [] } });
});
...
test('sidebar New split button: chat starts a chat, project takes a title inline',
  ...
  await new Promise(r => setTimeout(r, 5));
  assert.deepEqual(posts.pop(), { url: '/input', body: { text: '/new', attachments: [] } });
  ...
  assert.deepEqual(posts.pop(),
    { url: '/input', body: { text: '/project create Spaceship Engineering', attachments: [] } });
