...
const tmps = [];
function tmp(tag) {
  const d = fs.mkdtempSync(path.join(os.tmpdir(), `bfp-sg-${tag}-`));
  tmps.push(d);
  // realpath, always: macOS's /var/folders is really /private/var/folders and
  // blog.mjs resolves both sides of every comparison
...
// --- server harness ------------------------------------------------------
const spawned = [];
const SECRETS = fs.mkdtempSync(path.join(os.tmpdir(), 'bfp-sg-secrets-'));
