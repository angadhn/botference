 ...
 * they highlighted a paragraph in a browser — and the bot is editing MARKDOWN,
...
export function blogBlock(blog) {
  ...
  return `[blog draft: ${blog.url_path} · source ${blog.source_path}]\n`
    ...
    + `the prose without its markdown. Find the matching passage in the source yourself and edit `
    + `it there — the front matter, the markdown body, and the image lines are all yours.\n`
    + `WHERE YOU MAY WRITE: ${blog.source_path}, and the image and attachment files this post `
    + `uses under ${assets} — new images may be added `
    ...
    + `Jekyll rebuilds by itself and the reader's tab reloads, so make the edit and say what you `
    + `changed. If your change also touched the post somewhere else, add one line per place: `
    + `\`also changed — this passage now reads: "…"\`, with the full new wording.\n`;
}
