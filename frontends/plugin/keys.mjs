...
//
// Discuss drives the `claude` and `codex` CLIs, and those CLIs already know how
// to prefer an API key over the login they hold: give the child process
// ANTHROPIC_API_KEY / OPENAI_API_KEY and it bills the key; leave the variable
// out and it uses whatever the user logged in with. So there is nothing to
// implement in the protocol — the whole feature is deciding what the bridge's
// environment says at spawn, per agent.
//
...
// mode says what happens, and a variable left over in someone's shell is not
// an answer the user chose.
//
...
import { secretsDir } from './identity.mjs';
...
//           make, not something a companion should do behind their back.
//           (`forced_login_method = "api"` DOES force it — and silently
//            deletes ~/.codex/auth.json, logging you out. Never send it.)
//
// So for codex a stored key is a fallback for the logged-out case, and the UI
// says as much rather than promising an override the CLI will not perform.
export const KEY_OVERRIDES_LOGIN = { claude: true, codex: false };

export const keysFile = () => path.join(secretsDir(), 'discuss-keys.json');

const blank = () => ({ keys: {}, modes: {} });

// mtime-watched like grants.json: edited by hand or by another process, and
// picked up without a restart.
let cache = { mtime: -1, size: -1, data: blank() };
function read() {
  let st = null;
  try { st = fs.statSync(keysFile()); } catch { cache = { mtime: -1, size: -1, data: blank() }; return cache.data; }
  if (st.mtimeMs !== cache.mtime || st.size !== cache.size) {
    let data = blank();
    try {
      const j = JSON.parse(fs.readFileSync(keysFile(), 'utf8'));
      if (j && typeof j === 'object') {
        data = { keys: (j.keys && typeof j.keys === 'object') ? j.keys : {},
...
function write(data) {
  ...
  cache = { mtime: -1, size: -1, data: blank() }; // re-read on the next call
}

const isAgent = a => AGENTS.includes(String(a));
export const modeOf = (agent) => {
  const m = read().modes[agent];
  return MODES.includes(m) ? m : 'auto';
};
const keyOf = (agent) => {
  const k = read().keys[agent];
  return (typeof k === 'string' && k.trim()) ? k.trim() : '';
};

// Everything the UI is allowed to know: whether a key exists, not what it is.
export function status() {
  const out = { modes: {} };
  for (const a of AGENTS) {
    out[a] = keyOf(a) ? 'set' : 'unset';
    out.modes[a] = modeOf(a);
  }
  return out;
}
...
export function setKey(agent, key) {
  ...
  const data = read();
  write({ keys: { ...data.keys, [agent]: v }, modes: { ...data.modes } });
  return { ok: true };
}

// Removal is a real operation, not an overwrite: the key leaves the file, and
// with mode 'auto' the very next bridge gets no variable at all.
export function removeKey(agent) {
  ...
  delete keys[agent];
  write({ keys, modes: { ...data.modes } });
  return { ok: true, removed: had };
}

export function setMode(agent, mode) {
  if (!isAgent(agent)) return { ok: false, error: 'unknown agent' };
  if (!MODES.includes(String(mode))) return { ok: false, error: `mode must be one of ${MODES.join(', ')}` };
  const data = read();
  write({ keys: { ...data.keys }, modes: { ...data.modes, [agent]: String(mode) } });
  return { ok: true };
}
